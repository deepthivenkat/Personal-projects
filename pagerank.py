...
This program
(1) initializes the memory-mapped objects (index_map and edge_map),
(2) unpacks the source and target IDs from the edge_map, and
(3) upacks the source ID and source degree from the index_map.

This code assumes that the node IDs start from 0 and are contiguous up to max_node_id.
Your algorithm returns the same scores
